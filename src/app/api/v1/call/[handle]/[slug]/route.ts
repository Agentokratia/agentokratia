...
import {
  verifyPayment, settlePayment, simulatePayment, decodePaymentSignatureHeader,
...
const SKIP_HEADERS = new Set([
  'host', 'connection', 'content-length', 'transfer-encoding', 'authorization', 'cookie',
  'x-payment', 'x-payment-required', 'x-payment-response',
  'x-agentokratia-request-id', 'x-agentokratia-agent-id', 'x-agentokratia-caller',
  'x-agentokratia-timestamp', 'x-agentokratia-secret',
]);
...
const jsonResponse = (body: object, status: number, headers: Record<string, string> = {}) =>
  new NextResponse(JSON.stringify(body), { status, headers: { 'Content-Type': 'application/json', ...CORS, ...headers } });
...
async function getAgentByHandleSlug(handle: string, slug: string): Promise<{
  ...
  // Validate endpoint
  if (!data.endpoint_url || data.endpoint_url === 'https://placeholder.example.com' || isInternalUrl(data.endpoint_url)) {
    return null;
  }
...
function buildPaymentRequired(agent: { name: string; price_per_call: number }, ownerWallet: string, resource: string, networkConfig: NetworkConfig): PaymentRequired {
  return {
    x402Version: X402_VERSION,
    resource: { url: resource, description: `API call to ${agent.name}`, mimeType: 'application/json' },
    accepts: [{
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ handle: string; slug: string }> }) {
  ...
  if (!paymentHeader) {
    ...
    return jsonResponse(
      { error: 'Payment required', message: `This API costs ${priceUsdc} USDC per call`, agentId: agent.id, agentName: agent.name, priceUsdc },
      402, { [X402_HEADERS.PAYMENT_REQUIRED]: encodePaymentRequiredHeader(paymentRequired), 'X-Agentokratia-Request-Id': requestId }
    );
  }
  ...
  catch { return errorResponse('Invalid payment header format', 400, requestId); }

  const paymentReqs = paymentPayload.accepted;
  const expected = paymentRequired.accepts[0];
  if (paymentReqs.amount !== expected.amount || paymentReqs.payTo.toLowerCase() !== expected.payTo.toLowerCase() ||
      paymentReqs.network !== expected.network || paymentReqs.asset.toLowerCase() !== expected.asset.toLowerCase()) {
  ...
  if (!verifyResult.isValid) {
    return jsonResponse({ error: 'Payment verification failed', reason: verifyResult.invalidReason }, 402,
      { [X402_HEADERS.PAYMENT_REQUIRED]: encodePaymentRequiredHeader(paymentRequired), 'X-Agentokratia-Request-Id': requestId });
  }
  ...
  try {
    ...
    if (body.length > MAX_REQUEST_SIZE) return errorResponse('Request body too large', 413, requestId);
  } catch { return errorResponse('Failed to read request body', 400, requestId); }
  ...
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), Math.min(agent.timeout_ms || DEFAULT_TIMEOUT_MS, MAX_TIMEOUT_MS));
    try {
      targetResponse = await fetch(agent.endpoint_url, { method: 'POST', headers: targetHeaders, body: body || undefined, signal: controller.signal });
  ...
  let settleResult: Awaited<ReturnType<typeof settlePayment>> | null = null, lastError: string | undefined;
  for (let i = 1; i <= 3; i++) {
